...
void Coastline::Initialise(int NodeSpacing, double CoastLength, double Trend, int StartBoundaryInput, int EndBoundaryInput)
{
	...
	//setup fixed nodes at start and end for fixed boundaries
	if (StartBoundary == 2 && EndBoundary == 2)
	{
		for (int i=0; i<NoNodes; ++i)
		{	
			{
				if ((i < 10) || (i > NoNodes-1)) Fixed[i] = 1;
				else Fixed[i] = 0;
			}
		}
	}
	...
	*/
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		if (Fixed[i] == 0) 
		{
			...
			if (Fixed[i-1] == 1 && LongshoreFlux[i] < 0) LongshoreFlux[i-1] = LongshoreFlux[i];
			VolumeChange[i] = (LongshoreFlux[i-1]-LongshoreFlux[i])*(1.-LostFluxFraction);
			if (Fixed[i+1] == 1 && LongshoreFlux[i] > 0) VolumeChange[i] = 0;
		}
		else VolumeChange[i] = 0;
		
//		if (Fixed[i] == 1)
//		{
//			//nothing gets washed in, only washed out
//			VolumeChange[i] = 0;
//			if (i <= 1 && LongshoreFlux[i] > 0) LongshoreFlux[i] = 0;
//			else if (i==NoNodes-3 && LongshoreFlux[i] < 0) 
//			{	
//				LongshoreFlux[i] = 0;
//				VolumeChange[i] = (LongshoreFlux[i-1]-LongshoreFlux[i])*(1.-LostFluxFraction);
//			}
...
//		else VolumeChange[i] = (LongshoreFlux[i-1]-LongshoreFlux[i])*(1.-LostFluxFraction);		
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		if (i==0) e1 = Orientation3Cell[i]-Orientation2Cell[i];
		else e1 = Orientation3Cell[i]-Orientation2Cell[i-1];
