...
void Coastline::Initialise(string xyfilename)
{
	...
	EmptyVector = Empty;
	Orientation3Cell = EmptyVector;
	Orientation2Cell = EmptyVector;
...
{
	/************************************************************
	Calculates Orientation2Cell [i:i+1], Orientation3Cell [i-1:i+1], 
	members of Coastline object from X and Y positions

	Orientation3Cell is the direction of vector i-1 to i+1
	Orientation2Cell is the direction of vector i to i+1
	...
	************************************************************/
	...
	if (NodesToRemove.size() > 0)
	{
		...
		{
			X.erase(X.begin()+NodesToRemove[j]);
			Y.erase(Y.begin()+NodesToRemove[j]);
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		if (i==0)
		{
			...
			//convert to azimuths
			if (dX == 0 && dY < 0) Orientation2Cell[i] = 180.;
			else if (dX == 0 && dY > 0) Orientation2Cell[i] = 0.;
			else if (dX > 0) Orientation2Cell[i] = (180./M_PI)*(M_PI*0.5 - atan(SpatialGradient));
			else if (dX < 0) Orientation2Cell[i] = (180./M_PI)*(M_PI*1.5 - atan(SpatialGradient));
			else
			{
			...
			//convert to azimuths
			if (dX == 0 && dY < 0) Orientation3Cell[i] = 180.;
			else if (dX == 0 && dY > 0) Orientation3Cell[i] = 0.;
			else if (dX > 0) Orientation3Cell[i] = (180./M_PI)*(M_PI*0.5 - atan(dY/dX));
			else if (dX < 0) Orientation3Cell[i] = (180./M_PI)*(M_PI*1.5 - atan(dY/dX));
			else cout << "c'est impossible!" << endl;
			
			//calculate cell width
			CellWidth[i] = (sqrt(pow(X[i+1]-X[i],2.0) + pow(Y[i+1]-Y[i],2.0)))/cos(M_PI/180.*(Orientation3Cell[i]-Orientation2Cell[i]));
		}
		...
		else if (i == NoNodes-1)
		{
			...
			{
				//First get flux orientation (2-node)
				Orientation2Cell[i] = Orientation2Cell[0];
				Orientation3Cell[i] = Orientation3Cell[0];
				CellWidth[i] = CellWidth[0];
			}
			//FIXED BOUNDARY
			else if (EndBoundary == 2)
			{
				Orientation2Cell[i] = Orientation2Cell[i-1];
				Orientation3Cell[i] = Orientation3Cell[i-1];
				CellWidth[i] = (sqrt(pow(X[i]-X[i-1],2.0) + pow(Y[i]-Y[i-1],2.0)))/cos(M_PI/180.*(Orientation3Cell[i]-Orientation2Cell[i]));
			}
			//BOUNDARY ERROR
			else
			{
		...
		else
		{
			...
			CellWidth[i] = (0.5*sqrt(pow(X[i]-X[i-1],2.0) + pow(Y[i]-Y[i-1],2.0)))/cos(M_PI/180.*(Orientation3Cell[i]-Orientation2Cell[i-1]));
			CellWidth[i] += (0.5*sqrt(pow(X[i+1]-X[i],2.0) + pow(Y[i+1]-Y[i],2.0)))/cos(M_PI/180.*(Orientation3Cell[i]-Orientation2Cell[i]));
		}
	}
}
...
void Coastline::CheckNodeSpacing()
{
	...
	while (SpacingFlag == 0)
	{
		for (int i=0; i<NoNodes-1; ++i)
		{
			...
			{	
//				Distance = sqrt(pow(X[i+1]-X[i-1],2.0) + pow(Y[i+1]-Y[i-1],2.0));
//				
//				//if distance between two adjacent cells is too small, 
//				//replace with a linearly interpolated point between
//				if (Distance < DesiredNodeSpacing)
//				{
...
void Coastline::TransformWave(int i)
{
	...
	double C_0, L_0, Alpha_0, Alpha_0_Last, H_0, Theta_0;
	double C, H, L, Alpha;
	double h, n, k, Ks, Kr;
	...
	//h = ClosureDepth;
	//Determine alpha_0
	//Use Orientation2Cell[i-1] for high angle waves
	//UseOrientation2Cell[i] for low angle waves (Ashton et al. 2006 uses Orientation3Cell)
	//Use +/- 45 degrees for the transition (like Ashton et al. 2006)
	...
	if(Shadows[i] == 4)
	{
		...
		if (Theta_0 <= Orientation3Cell[i+1]) Alpha_0 = Orientation2Cell[i+1]-Theta_0-90.;
    	else if (Theta_0 > Orientation3Cell[i+1]+270.) Alpha_0 = (Orientation2Cell[i+1]+270.)-Theta_0;
	   	else Alpha_0= 270.-(Theta_0-Orientation2Cell[i+1]);
	}
	else if (Shadows[i] == 2)
	{
		//determine alpha_0 angle between coast and wave crest approach for this and next cell!
		//cout << "Haven't cheked this" << endl;
		if (Theta_0 <= Orientation3Cell[i]) Alpha_0 = Orientation2Cell[i-1]-Theta_0-90.;
    	else if (Theta_0 > Orientation3Cell[i]+270.) Alpha_0 = (Orientation2Cell[i-1]+270.)-Theta_0;
	   	else Alpha_0= 270.-(Theta_0-Orientation2Cell[i-1]);
	}
	else
	{
		//determine alpha_0 angle between coast and wave crest approach
		if (Theta_0 <= Orientation3Cell[i]) Alpha_0 = Orientation2Cell[i]-Theta_0-90.;
    	else if (Theta_0 > Orientation3Cell[i]+270.) Alpha_0 = (Orientation2Cell[i]+270.)-Theta_0;
	   	else Alpha_0 = 270.-(Theta_0-Orientation2Cell[i]);
		
		if (i > 0)
		{
	   		if (Theta_0 <= Orientation3Cell[i-1]) Alpha_0_Last = Orientation2Cell[i-1]-Theta_0-90.;
    		else if (Theta_0 > Orientation3Cell[i-1]+270.) Alpha_0_Last = (Orientation2Cell[i-1]+270.)-Theta_0;
	   		else Alpha_0_Last = 270.-(Theta_0-Orientation2Cell[i-1]);
	   	}
	   	else Alpha_0_Last = Alpha_0;
	    
	   //if transitioning from low angle waves to high angle waves then use alpha_0 = 45.0
	   if ((Alpha_0_Last < 45. && Alpha_0 > 45.) 
	   		|| (Alpha_0 > -45. && Alpha_0_Last < -45.)
	   		|| (Alpha_0_Last > 45. && Alpha_0 < 45.)
	   		|| (Alpha_0 < -45. && Alpha_0_Last > -45.))
	   		Alpha_0 = 45.;
	   else if (Alpha_0 > 45.0)
	   {
	    	if (Theta_0 <= Orientation3Cell[i]) Alpha_0 = Orientation2Cell[i-1]-Theta_0-90.;
	    	else if (Theta_0 > Orientation3Cell[i]+270.) Alpha_0 = (Orientation2Cell[i-1]+270.)-Theta_0;
		   else Alpha_0= 270.-(Theta_0-Orientation2Cell[i-1]);
	...
	//cout << "Inside transform waves" << endl;
	...
	*/
	...
	for (i=1; i<NoNodes-1; ++i)
	{
		...
		if (Shadows[i] < 1)
		{
            ...
            if ((ShadowTest <= 0) && (abs(ShadowAngle-Orientation2Cell[i]) < 90.))
            {
     ...
     for (I=NoNodes-2; I>1; --I)
     {
        if (Shadows[I] < 1)
        {
            ...
            TempOrientation = Orientation2Cell[I-1]+180.;
	...
	*/
	...
	for (i=1; i<NoNodes-1; ++i)
	{
		...
		if (Shadows[i] == 2)
		{
			...
			TransformWave(j);
			TempWaveAngle = Orientation2Cell[j]-90.-BreakingWaveAngle[j];
			...
			else
			{
				...
				else
				{
					...
					Y.erase(Y.begin()+i+1, Y.begin()+j+1);
						EmptyVector.erase(EmptyVector.end()-(j-i),EmptyVector.end());
						NoNodes -= (j-i);
				}
			}
		}
...
void Coastline::TransportSediment(double &TimeDelta, double MaxTimeStep, Wave TheWave, int FluxType)
{
	...
	*/
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		//dt = (MeanNodeSpacing/fabs(VolumeChange[i]));
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		if (i==0) e1 = Orientation2Cell[i]-Orientation2Cell[i];
		else e1 = Orientation2Cell[i]-Orientation2Cell[i-1];
		e2 = Orientation2Cell[i]-Orientation3Cell[i];
		...
		if ((PositionChange[i] != PositionChange[i]) 
			|| (fabs(PositionChange[i] > 100.))
			|| ((Orientation2Cell[i-1]-Orientation2Cell[i]) > 120.))
		{
//			LongshoreFlux[i] = 0;
...
//			else PositionChange[i] = 0;
		...
		X[i] -= PositionChange[i]*cos((M_PI/180.)*Orientation3Cell[i]);
		Y[i] += PositionChange[i]*sin((M_PI/180.)*Orientation3Cell[i]);
	}
	...
	CalculateMorphology();
			...
			//No change assume sediment bypassing??
			//or assume locally rectilinear cell!?
			//PositionChange[i] = VolumeChange[i]/(ClosureDepth*CellWidth[i]);
...
//			else
//			{
//				//assume locally rectilinear cell?
//				PositionChange[i] = VolumeChange[i]/(ClosureDepth*CellWidth[i]);
//			}
//				int c2 = c;
//				int n = 0;
//				//or move half the sand and bypass half the sand
//				//this will only work in one direction!?
//				while (PositionChange[i] != PositionChange[i])
//				{	
//					c2 = c2/2.;
//					++n;
//					PositionChange[i] = (-b+sqrt((b*b)-(4.*a*c)))/(2.*a);
//					LongshoreFlux[i] = LongshoreFlux[i] - LongshoreFlux[i]/(2.*n) + LongshoreFlux[i-1]/(2.*n);
//				}
//				//cout << "I am not equal to myself!!" << endl;
				//cout << "Found NaN in Position Change: Node is " << i << endl;
			//}
...
void Coastline::CalculateFlux(int i, int FluxType)
{
	...
	*/
	...
	if (FluxType == 0)
	{
		...
		double DiffCoef = 1.0;
		LongshoreFlux[i] = DiffCoef*Orientation2Cell[i];
	...
	else if (FluxType == 1)
	{
		...
		if (LongshoreFlux[i] != LongshoreFlux[i])
		{
			//Check for NaN in PositionChange
			cout << "I am not equal to myself!!" << endl;
			cout << "Found NaN in Longshore Flux: Node is " << i << endl;
		}
	}
	else if (FluxType == 2)
	{
...
void Coastline::SimpleDiffusion(double DiffCoeff, double TimeDelta)
{
	...
	//temp variables
	double PositionChange, dX, dY, Curvature;
	...
	for (int i=0; i<NoNodes; ++i)
	{
		if (i > 1 || i < NoNodes-2)
		{
			...
			dX = (X[i+1]-X[i-1]);
			dY = (Y[i+1]-Y[i-1]);
			Curvature = (tan((M_PI/180.)*(Orientation2Cell[i]-Orientation2Cell[i-1])))/sqrt(dX*dX + dY*dY);
			PositionChange = -DiffCoeff*Curvature*TimeDelta;
		
			//Decompose Position change into X and Y components
			//Going to need a bit more logic here to handle different orientations
			//current setup x change is negative, y change is positive
			X[i] -= PositionChange*cos((M_PI/180.)*Orientation3Cell[i]);
			Y[i] += PositionChange*sin((M_PI/180.)*Orientation3Cell[i]);
		}
	}
