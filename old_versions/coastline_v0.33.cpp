...
void Coastline::BuildCellGeometries()
{
	...
	*/
	...
	double O1, O2, M1, M2, X1, Y1, X2, Y2, Wb, Dsf, Dsfi, Dsfa, Dsfb, Lsfi, Lsfa, Lsfb, Sum, L, dX, dY, SpatialGradient, TempOrientation, TempAngle, X0temp, Y0temp;
	...
	//if ((StartBoundary == 2) && (EndBoundary == 2)) CellType[0] = 0, CellType[1] = 0, CellType[NoNodes-1] = 0, CellType[NoNodes-2] = 0;
...
//					continue;
		      ...
		      if (Dsfa < 0 || ((StartBoundary == 2) && (a < 2))) Dsfa = ClosureDepth;
				if (Dsfb < 0 || ((EndBoundary == 2) && (b > NoNodes-3))) Dsfb = ClosureDepth;
	...
	for (i=0; i<NoNodes; ++i)
	{
		...
		if (i <= 1 || i>=NoNodes-3)
		{
			...
			else if (StartBoundary == 2 && EndBoundary == 2)
			{
				...
				{
					if (LongshoreFlux[i] < 0) LongshoreFlux[i] = LongshoreFlux[NoNodes-1]*FluxFraction + 10.*(-0.5+((double)rand()/RAND_MAX));
