...
//=========================================================================
//
// 24/7/15 v0.41 Major overhaul of cell building algorithm
//
...
void Coastline::Initialise(int NodeSpacing, double CoastLength, double TheTrend, int StartBoundaryInput, int EndBoundaryInput)
{
  ...
  double NoiseAmplitude = (MeanNodeSpacing/5.);
	...
	double TempSpacing = Dist-CoastLength;
 	tempX = tempX + TempSpacing*sin(Trend*M_PI/180); // + NoiseAmplitude*(-0.5+((double)rand()/RAND_MAX));
	tempY = tempY + TempSpacing*cos(Trend*M_PI/180); // + NoiseAmplitude*(-0.5+((double)rand()/RAND_MAX));
	X.push_back(tempX);
	Y.push_back(tempY);
	NoNodes += 1;
      ...
      if (CellType[i] == 1 && Dsft<ClosureDepth)
      {
        ...
        while (CellType[b] == 0)
        {
          ...
          ++b;
				...
				M1 = 1./tan((M_PI/180.)*O1);
			  M2 = 1./tan((M_PI/180.)*O2);
			  if (X0[i-1] == -9999) X0[i-1] = XL[i], Y0[i-1] = YL[i];
			  if (X0[i+1] == -9999) X0[i+1] = XR[i], Y0[i+1] = YR[i];
