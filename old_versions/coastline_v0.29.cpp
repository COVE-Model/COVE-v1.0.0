...
void Coastline::Initialise(string xyfilename)
{
	...
	vector< vector<Node> > EmptyVertices(NoNodes);
...
void Coastline::Initialise(string xyfilename, int StartTime)
{
	...
	vector< vector<int> > EmptyRecievers(NoNodes);
	Recievers = EmptyVertices;
		...
		else if (i==NoNodes-1)
		{
         ...
         YL[i] = Y[i]+LeftDiff*sin((M_PI/180.)*(Orientation[i]-90.));
			...
			if (EndBoundary == 1)
			{
				...
				MeshOrientation[i] = MeshOrientation[0];
			...
			else if (EndBoundary == 2)
			{
				...
				MeshOrientation[i] = MeshOrientation[i-1];
				Curvature[i] = 0;
...
void Coastline::BuildCellGeometries()
{
	...
	*/
	...
	double O1, O2, M1, M2, X1, Y1, X2, Y2, Wb, Dsf, Dsfa, Dsfb, Lsfa, Lsfb, Sum, DistL, DistR, L, Xtemp, Ytemp;
	int i, a, b;
	...
	while (!CoastlineQueue.empty())
	{
		...
		if (i<2 || i>NoNodes-3 || Dsf<=0 || CellType[i]==0) 
		{
			Vertices[i].clear();
			continue;
		}
		else 
		{
			if (CellType[i] == 1 && Dsf<ClosureDepth-0.001)
			{
				...
				Y0[i] = M1*X0[i] + YL[i] - M1*XL[i];
		     	...
		     	//add X0,Y0 as first point in vertices list for i+1 and i-1
		     	Vertices[b].push_back(XYNode), Vertices[a].push_back(XYNode);
		     	...
		     	MeshOrientation[i] = Orientation[i];
		     	MeshOrientation[b] = Orientation[i];
		      ...
		      //find intersection
				...
				Lsfb = (sqrt((X0[i]-XR[i])*(X0[i]-XR[i]) + (Y0[i]-YR[i])*(Y0[i]-YR[i])))*cos((M_PI/180.)*(e2[i]));
		     	...
		     	if (Dsfb > ClosureDepth)
		     	{
		     	   ...
		     	   //find point of intersection with meshorientation
					O1 = MeshOrientation[i-1]-90.;
			      O2 = Orientation[i];
			      M1 = 1./tan((M_PI/180.)*(O1));
			      M2 = 1./tan((M_PI/180.)*(O2));
		         Xtemp = (Y0[i] - M2*X0[i] + M1*X0[i-1] - Y0[i-1])/(M1 - M2);
		         Ytemp = M1*Xtemp + Y0[i+1] - M1*X0[i-1];
		                     
		         //add nodes to vertices list and calculate area
		     		XYNode.X = Xtemp, XYNode.Y = Ytemp, Vertices[i].push_back(XYNode);
			     	XYNode.X = XR[i], XYNode.Y = YR[i], Vertices[i].push_back(XYNode);
			     	XYNode.X = XL[i], XYNode.Y = YL[i], Vertices[i].push_back(XYNode);
			   ...
			   XYNode.X = XL[i], XYNode.Y = YL[i], Vertices[i].push_back(XYNode);
				...
				//Update mesh orientation as mean orientation of all contributing cells
		     	MeshOrientation[i+1] = (MeshOrientation[i]*Weighting[i]+Orientation[i])/(Weighting[i]+1.);
			   Weighting[i+1] = Weighting[i]+1;
			...
			else if (CellType[i] == 4)
			{
				...
				//find extent of domain currently being processed and update CellType
			  	...
			  	//find intersection of cell margin a and b
			  	O1 = MeshOrientation[a]-90.;
				...
				//this might not work? does c++ queue have an insert function and what is its syntax?
				XYNode.X = X0[i], XYNode.Y = Y0[i], Vertices[i].insert(Vertices[i].begin()+1,XYNode);
				Vertices[b].push_back(XYNode); 
			  
			  	//Add XLYL XRYR to vertices
			  	DistL = sqrt((Vertices[i][2].X-XL[i])*(Vertices[i][2].X-XL[i]) + (Vertices[i][2].Y-YL[i])*(Vertices[i][2].Y-YL[i]));
			  	DistR = sqrt((Vertices[i][2].X-XR[i])*(Vertices[i][2].X-XR[i]) + (Vertices[i][2].Y-YR[i])*(Vertices[i][2].Y-YR[i]));
			   ...
			   //loop through vertices to calculate cell area
				...
				MeshOrientation[i] = (MeshOrientation[i+1]*Weighting[i+1]+MeshOrientation[i-1]*Weighting[i-1])
											/(Weighting[i-1]+Weighting[i+1]);
