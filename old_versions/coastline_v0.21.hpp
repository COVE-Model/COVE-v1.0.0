...
class Coastline
{
	...
	vector<double> RegionalOrientation;		//shoreline orientation bewteen i-1, i+2 (private)
	...
	// PRIVATE FUNCTIONS
	void CalculateMorphology();				//Function to calculate orientations and cell widths along the coast
	void CheckNodeSpacing();				//Checks if cells get too close or too far apart and adds/destroys
	void CalculateMeanNodeSpacing();		//think this is now redundant
	void IntersectionAnalysis();			//checks coastline doesn't intersect itself
	void FindBarrierCells();				//finds where the coastline gets too close to itself
	void TransformWave(int i);				//transforms offshore waves (including diffracted if in shadow zone)
	void TransformWaves();					//runs TransformWave on all cells
	void GetShadows();						//Find Shadow Zones
	void RefractDiffractShadowZone();		//Modifies offshore waves due to Diffraction in Shadowing
