...
//		if (		((OffshoreWaveDirection >= Orientation[i]) && (OffshoreWaveDirection <= Orientation[i]+180.0))
//				||	((OffshoreWaveDirection >= Orientation[i]) && (OffshoreWaveDirection <= Orientation[i]-180.0)))
//		{
...
//		
		//if cell is shadowed do not transport any sediment
//		if (Shadows[i] == 1)
//		{
////			BreakingWaveHeight[i] = 0;
////			BreakingWaveAngle[i] = 0;
...
//					BreakingWaveAngle[j] = (TempWaveAngle + 1.5*(180.-TempWaveAngle))+90.;
//					BreakingWaveHeight[j] = TempWaveHeight*cos((M_PI/180.)*(OffshoreWaveDirection-180.));
				}
				else if (dX == 0 && dY > 0) 
				{
...
//					BreakingWaveAngle[j] = (TempWaveAngle + 1.5*(360.-TempWaveAngle))+90.;
//					BreakingWaveHeight[j] = TempWaveHeight*cos((M_PI/180.)*(OffshoreWaveDirection-360.));
				}
				else if (dX > 0)
				{
					//cout << "NEED TO DO THIS BIT!" << endl;
					...
					//ShadowZoneWaveDirection[j] = fabs(90.-OffshoreWaveDirection+(180/M_PI)*((M_PI/180.)*(OffshoreWaveDirection))*log(1.+(exp(1.)-1.)*sin((M_PI/180.)*(((120.-(180./M_PI)*AngleDiff)*90.)/(180.-OffshoreWaveDirection))))-90.);
					...
					//ShadowZoneWaveHeight[j] = OffshoreWaveHeight*(2.*cos(0.8*AngleDiff)-1.);
					//needs updating to match dX < 0
//					ShadowZoneWaveDirection[j] = (TempWaveAngle + 1.5*((180./M_PI)*(M_PI*0.5 - atan(SpatialGradient))-TempWaveAngle))+90.;
//					ShadowZoneWaveHeight[j] = TempWaveHeight*cos(((M_PI/180.)*(OffshoreWaveDirection))-(M_PI*0.5-atan(SpatialGradient)));
//					BreakingWaveAngle[j] = (TempWaveAngle + 1.5*((180./M_PI)*(M_PI*0.5 - atan(SpatialGradient))-TempWaveAngle))+90.;
//					BreakingWaveHeight[j] = TempWaveHeight*cos(((M_PI/180.)*(OffshoreWaveDirection))-(M_PI*0.5-atan(SpatialGradient)));
				}
				else if (dX < 0) 
				{
					...
					ShadowZoneWaveHeight[j] = OffshoreWaveHeight*cos(AngleDiff)*cos(AngleDiff)*cos(AngleDiff);
					//ShadowZoneWaveHeight[j] = OffshoreWaveHeight*(2.*cos(0.8*AngleDiff)-1.);
					//ShadowZoneWaveHeight[j] = OffshoreWaveHeight*(2.*cos(AngleDiff/2.)-1.);
					//BreakingWaveAngle[j] = FluxOrientation[j] - (TempWaveAngle + 1.5*((((180./M_PI)*(M_PI*1.5 - atan(SpatialGradient)))-180.)-TempWaveAngle)+90.);
					//AngleDiff = (M_PI*1.5-atan(SpatialGradient))-((M_PI/180.)*(ShadowAngle));
					//BreakingWaveHeight[j] = TempWaveHeight*cos(AngleDiff)*cos(AngleDiff); //*(1.-sin(AngleDiff)*(D2/D1));
					
					//TempAngle = (((180./M_PI)*(M_PI*1.5-atan(SpatialGradient)))-ShadowAngle);
					// if (TempAngle <= 0) BreakingWaveHeight[j] = 0;
					//else BreakingWaveHeight[j] = TempWaveHeight*(1.-(90.-TempAngle)/90.);
				}
				else 
				{
				...
				//if (BreakingWaveAngle[j] > 360.) BreakingWaveAngle[j] -= 360.;
				if (ShadowZoneWaveDirection[j] == -9999 || ShadowZoneWaveHeight[j] == -9999)
				{
					cout << "Diffraction FAILS!" << endl;
				}
				
//				if (BreakingWaveHeight[j] < 0.) BreakingWaveHeight[j] = 0.;
//				//if (BreakingWaveAngle[j] > 360.) BreakingWaveAngle[j] -= 360.;
//				if (BreakingWaveAngle[j] == -9999 || BreakingWaveHeight[j] == -9999)
//				{
//					cout << "Diffraction FAILS!" << endl;
//				}
				--j;
...
void Coastline::TransportSediment(double &TimeDelta, double MaxTimeStep, Wave TheWave, int FluxType)
{
	...
	*/
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		if (BreakingWaveAngle[i] == -9999 || BreakingWaveHeight[i] == -9999)
		{
			cout << "Something wrong with Breaking Waves!" << endl;
			GetShadows();
			RefractDiffractShadowZone();
			TransformWaves();
		}
