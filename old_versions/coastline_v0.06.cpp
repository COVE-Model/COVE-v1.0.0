...
void Coastline::WriteCoast(string OutputFileName, double Time)
{
	...
	{
		//write X
		WriteCoastFile << setprecision(2) << Time;
...
//					//if cell is too narrow remove it and replace adjacent nodes with
//					//interpolated cells to midpoint
//					X[i-1] = ((X[i-1]+X[i])/2.);
//					Y[i-1] = ((Y[i-1]+Y[i])/2.);
//					X[i+1] = ((X[i]+X[i+1])/2.);
//					Y[i+1] = ((Y[i]+Y[i+1])/2.);
...
//					break;
				...
				(Distance > 2.0*DesiredNodeSpacing) 
				{
...
//		
		...
		{
			if (Theta_0 <= Orientation[i+1]) Alpha_0 = Orientation[i+1]-Theta_0-90.;
    		else if (Theta_0 > Orientation[i+1]+270.) Alpha_0 = (Orientation[i+1]+270.)-Theta_0;
	    	else Alpha_0= 270.-(Theta_0-Orientation[i+1]);
		}
		else
		{
...
void Coastline::GetShadows(double OffshoreWaveDirection)
{
	...
	for (i=1; i<NoNodes-1; ++i)
	{
		...
		if (Shadows[i] != 1)
		{
            ...
            if ((ShadowTest <= 0) && (abs(ShadowAngle-FluxOrientation[i]) < 45.))
            {
            	Shadows[i+1] = 1;
...
void Coastline::CERCDiffusion(double &TimeDelta, double MaxTimeStep, Wave TheWave)
{
	/* DESCRIPTION GOES HERE
	...
	*/
	...
	{
		//CERC Equation
		if (Shadows[i] == 0) LongshoreFlux[i] = 86400.*0.41*pow(BreakingWaveHeight[i],2.5)
		...
		if (i==0 && StartBoundary == 1)
		{
			...
			LongshoreFlux[NoNodes-2] = 86400.*0.41*pow(BreakingWaveHeight[NoNodes-2],2.5)
										*sin((M_PI/180.0)*BreakingWaveAngle[NoNodes-2])
										*cos((M_PI/180.0)*BreakingWaveAngle[NoNodes-2]);
