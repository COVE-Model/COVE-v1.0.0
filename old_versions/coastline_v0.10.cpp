...
//
// 29/1/14 v_0.09 Seperate out sediment flux calculations and moving of sediment
...
//
// 28/1/14 v0.08 Added function to check for coastline intersecting itself and flag
//
// 21/1/14 v_0.07 Shadows algorithm debugged.
//
// 15/1/14 	v_0.06 Shadows Algorithm improved to better handle complex coastlines
//
...
void Coastline::Initialise(string xyfilename)
{
	...
	ShadowZoneAngle = EmptyVector;
	ShadowZoneHeight = EmptyVector;
...
void Coastline::TransformWaves(double OffshoreWavePeriod, double OffshoreWaveDirection, double OffshoreWaveHeight)
{
	...
	T = OffshoreWavePeriod;		//Wave Period
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		//Catch offshore waves using 3-cell orientation
		// is this necessary if we're using shadow zones?
//		if (		((Theta_0 >= Orientation[i]) && (Theta_0 <= Orientation[i]+180.0))
//				||	((Theta_0 >= Orientation[i]) && (Theta_0 <= Orientation[i]-180.0)))
//		{
...
//		}
		...
		//Use FluxOrientation for low angle waves and FluxOrientation[i-1][i+1] for high angle waves
//		
		//if cell is shadowed use diffracted/refracted
		if (Shadows[i] == 1)
		{
			Theta_0 = ShadowZoneAngle[i];
			H_0 = ShadowZoneHeight[i];
		...
		//for all regular cells exposed to offshore waves
		else
		{
			//here we go!
			if (Theta_0 <= FluxOrientation[i]) Alpha_0 = FluxOrientation[i]-Theta_0-90.;
...
void Coastline::GetShadows(double OffshoreWaveDirection, double OffshoreWaveHeight)
{
	/*description goes here*/
	
	//declare temporary variables
	double ShadowAngle, X2, Y2, ShadowTest, TempOrientation, dX, dY, SpatialGradient, LineAzimuth;
	int i, I;
	
	//reset shadows
	vector<double> Zeros(NoNodes, 0);
	Shadows = Zeros;
	ShadowZoneAngle = Zeros;
	ShadowZoneHeight = Zeros;
	...
	for (i=1; i<NoNodes-1; ++i)
	{
		...
		if (Shadows[i] != 1 && Shadows[i] != 2)
		{
            ...
            if ((ShadowTest <= 0))	// && (abs(ShadowAngle-FluxOrientation[i]) < 90.))
            {
            	...
            	//calculate shadow zone angle, limit to <90 degrees from offshore angle
				dX = X[i+1]-X[i];
				dY = Y[i+1]-Y[i];
				SpatialGradient = dY/dX;
			
				//convert to azimuths // THIS AINT FINISHED
				if (dX == 0 && dY < 0) ShadowZoneAngle[i+1] = ShadowAngle + 1.5*(180.-ShadowAngle);
				else if (dX == 0 && dY > 0) ShadowZoneAngle[i+1] = 0.;
				else if (dX > 0) ShadowZoneAngle[i+1] = ShadowAngle + 1.5*((180./M_PI)*(M_PI*0.5 - atan(SpatialGradient))-ShadowAngle);
				else if (dX < 0) 
				{
					LineAzimuth = ((180./M_PI)*(M_PI*1.5 - atan(SpatialGradient)));
//					if (1.5*(LineAzimuth-ShadowAngle) > 90.) 
//					{
//						ShadowZoneAngle[i+1] = ShadowAngle+90.;
//						ShadowZoneHeight[i+1] = 0;
//					}
//					else 
//					{
						ShadowZoneAngle[i+1] = ShadowAngle + 1.5*(LineAzimuth-ShadowAngle) - 180.;
						ShadowZoneHeight[i+1] = OffshoreWaveHeight*cos((M_PI/180.)*1.5*(LineAzimuth-ShadowAngle));
//					}
				}
				else cout << "This is impossible!" << endl;
            	...
            	for (int j=i+2; j<NoNodes-1; ++j)
            	{
                    ...
                    if (ShadowTest <= 0) 
                    {
						...
						//convert to azimuths // THIS AINT FINISHED
						if (dX == 0 && dY < 0) ShadowZoneAngle[j] = OffshoreWaveDirection + 1.5*(180.-OffshoreWaveDirection);
						else if (dX == 0 && dY > 0) ShadowZoneAngle[j] = 0.;
						else if (dX > 0) ShadowZoneAngle[j] = OffshoreWaveDirection + 1.5*((180./M_PI)*(M_PI*0.5 - atan(SpatialGradient))-OffshoreWaveDirection);
						else if (dX < 0) 
						{
							LineAzimuth = ((180./M_PI)*(M_PI*1.5 - atan(SpatialGradient)));
//							if (1.5*(LineAzimuth-ShadowAngle) > 90.) 
//							{
//								ShadowZoneAngle[j] = ShadowAngle+90.;
//								ShadowZoneHeight[j] = 0;
//							}
//							else
//							{
								ShadowZoneAngle[j] = ShadowAngle + 1.5*(LineAzimuth-ShadowAngle) - 180.;
								ShadowZoneHeight[j] = OffshoreWaveHeight*cos((M_PI/180.)*1.5*(LineAzimuth-ShadowAngle));
//							}
						}
						else cout << "This is impossible!" << endl;
        ...
        if (Shadows[I] != 1 && Shadows[I] != 2)
        {
            ...
            if ((ShadowTest >= 0)) // && (abs(ShadowAngle-TempOrientation) < 90.)))
            {
               ...
               //calculate shadow zone angle
				...
				//convert to azimuths // THIS AINT FINISHED
				if (dX == 0 && dY < 0) ShadowZoneAngle[I-2] = OffshoreWaveDirection + 1.5*(180.-OffshoreWaveDirection);
				else if (dX == 0 && dY > 0) ShadowZoneAngle[I-2] = 0.;
				else if (dX > 0) ShadowZoneAngle[I-2] = OffshoreWaveDirection + 1.5*((180./M_PI)*(M_PI*0.5 - atan(SpatialGradient))-OffshoreWaveDirection);
				else if (dX < 0) ShadowZoneAngle[I-2] = OffshoreWaveDirection + 1.5*((180./M_PI)*(M_PI*1.5 - atan(SpatialGradient))-OffshoreWaveDirection);
				else cout << "This is impossible!" << endl;
               ...
               //Continue through nodes to test if still in shadow
            	for (int J=I-2; J>0; --J)             
                {
                    ...
                    if (ShadowTest >= 0) 
                    {
						...
						else if (dX < 0) ShadowZoneAngle[J] = OffshoreWaveDirection + 1.5*((180./M_PI)*(M_PI*1.5 - atan(SpatialGradient))-OffshoreWaveDirection);
						else cout << "This is impossible!" << endl;
					}
               else 
               {
...
void Coastline::TransportSediment(double &TimeDelta, double MaxTimeStep, Wave TheWave, int FluxType)
{
	...
	*/
	...
	double OffshoreWavePeriod = TheWave.Get_WavePeriod();
	double OffshoreWaveHeight = TheWave.Get_WaveHeight();
	double OffshoreWaveDirection = TheWave.Get_WaveDirection();
	
	//Check Node Spacing is ok
	CheckNodeSpacing();

	//Find Shadows
	GetShadows(OffshoreWaveDirection, OffshoreWaveHeight);
	
	//Only waves if < 10 degrees from trend of coast
	//This won't work for islands but shouldnt be too much of a problem
	if 		((fabs(Trend-OffshoreWaveDirection) < 10.) 
		|| 	(fabs(Trend-180.-OffshoreWaveDirection) < 10.) 
		||	(fabs(Trend+180.-OffshoreWaveDirection) < 10.));
	//Transform waves
	else 
	{
		...
		for (int i=0; i<NoNodes; ++i)
		{
			...
			if (Shadows[i] != 1) CalculateFlux(i, FluxType);											
			...
			{
				//no transport, no volume change, fixed boundary
				LongshoreFlux[i] = 0;
				VolumeChange[i] = 0;
			}
			...
			//dt = 0.1*(MeanNodeSpacing/abs(VolumeChange[i]));
