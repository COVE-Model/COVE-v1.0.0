		...
		for (int i=0; i<NoNodes-1; ++i)
		{
			...
			{	
				...
				(Distance > 1.5*DesiredNodeSpacing) 
				{
					...
					//if (i == 1) cout << "Removing Node 1!" << endl;
					//if (i == NoNodes-2) cout << "Removing Node NoNodes-2!" << endl;
					break;
				}
				else
				{
...
//	//use "headland" orientation
	...
	else
	{
	    ...
	    if (i > 0)
		{
	   	...
	   	//first if transitioning from low to high angle use maximising angle
	    if (Alpha_0_Last < 45. && Alpha_0 > 45.) Alpha_0 = 45.;
	    else if (Alpha_0_Next > -45. && Alpha_0 < -45.) Alpha_0 = -45.;
	    
		//second if high angle use fluxorientation of previous 
	   	if (Alpha_0 > 45.0)
	    {
	    	if (Theta_0 <= Orientation[i]) Alpha_0 = FluxOrientation[i-1]-Theta_0-90.;
	    	else if (Theta_0 > Orientation[i]+270.) Alpha_0 = (FluxOrientation[i-1]+270.)-Theta_0;
...
void Coastline::RefractDiffractShadowZone()
{
	...
	*/
	...
	//declare temporary variables
	double ShadowAngle, SpatialGradient, ShadowZoneDistance, dX, dY, Dist, AngleDiff;
	int i, j, k;
	...
	for (i=1; i<NoNodes-1; ++i)
	{
		...
		if (Shadows[i] == 2)
		{
			...
			k = j;
			--j;
			ShadowZoneDistance = Distance[j]-Distance[i];
			...
			while(Dist <= ShadowZoneDistance)
			{
				...
				k+=1;
				Dist += (Distance[k]-Distance[k-1]);
				if (k>NoNodes-2 || k<1) Dist = ShadowZoneDistance+1;
			}
		}
		...
		else if (Shadows[i] == 3)
		{
			...
			//get distance the length of the shore across the shadow zone
			ShadowZoneDistance = Distance[i]-Distance[j];
			...
			while(Dist < ShadowZoneDistance)
			{
				...
				k-=1;
				if (k<1 || k >NoNodes-2) Dist = ShadowZoneDistance+1;
			}
		}
	}
	ShadowZoneWaveHeight[NoNodes-3] = 0;
}
			...
			else
			{
				...
				else
				{
					...
					if (fabs(Orientation[j]-Orientation[i]) > 90.)
					{
						...
						X[j+1] += 10.*PositionChange[i]*cos((M_PI/180.)*Orientation[j]);
						Y[j+1] -= 10.*PositionChange[i]*sin((M_PI/180.)*Orientation[j]);
...
void Coastline::FindBarrierCells()
{
	/*description goes here
	
	Function to loop through the coastline vector and find nearest cell
	Highlighting cells that have cells close than their updrift and downdrift cells
	...
	Martin Hurst, 19/2/2014
	
	*/
	
	//declare temporary variables
	double X1, X2, Y1, Y2, dX, dY, Dist;
	double MinBarrierWidth = 10.;
	...
	for (int i=0; i<NoNodes-2; ++i)
	{
		...
		for (int j=i+2; j<NoNodes-1; ++j)
		{
			...
			//if Dist is < DesiredNodeSpacing then a barrier
			if (Dist < MinBarrierWidth) 
			{
				//Need some stuff here to deal with barriers
				//i.e. maintain minimum barrier width to shadowed cell
				//cout << "I am a barrier" << endl;
				if (Shadows[j] == 1)
				{
					cout << "Updating barrier" << endl;
					X[j] += cos((M_PI/180.)*Orientation[j]);
					Y[j] -= sin((M_PI/180.)*Orientation[j]);
				}
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		if (i <= 1 || i>=NoNodes-3)
		{
			...
			else if (StartBoundary == 2 && EndBoundary == 2)
			{
				...
				else if (i == 2 && LongshoreFlux[i] < 0) LongshoreFlux[i-1] = LongshoreFlux[i];
				else if (i>NoNodes-3)
				{
				...
				{
					if (LongshoreFlux[NoNodes-1] < 0)
					{
						LongshoreFlux[i] = LongshoreFlux[NoNodes-1]*FluxFraction;
						VolumeChange[i] = (LongshoreFlux[i-1]-LongshoreFlux[i])*(1.-LostFluxFraction)*TimeDelta;
					}
					else if (LongshoreFlux[i] < 0) 
					{
						LongshoreFlux[i] = 0;
						VolumeChange[i] = (LongshoreFlux[i-1]-LongshoreFlux[i])*(1.-LostFluxFraction)*TimeDelta;
					}
		...
		//e1 and e2 are the left and right angles of the trapezium
...
//			}
	...
	if (PositionChange[NoNodes-2] != 0 || PositionChange[1] != 0)
	{
		cout << "Fixed Node Moved!" << endl;
	}
