...
void Coastline::GetShadowsNew()
{
	...
	*/
	...
	//declare temporary variables
	double X1, Y1, X2, Y2, dX12, dY12, X3, Y3, X4, Y4, dX34, dY34, dX31, dY31, S, T;
	double ShadowAngle;
	int i, CasterFlag;
	...
	for (i=0; i<NoNodes-1; ++i)
	{
	    ...
	    else
	    {
	        ...
	        X2 = X[i] + 100000.*np.sin((np.pi/180.)*OffshoreWaveDirection);
    	    Y2 = Y[i] + 100000.*np.cos((np.pi/180.)*OffshoreWaveDirection);
           	dX12 = X2-X1;
	        dY12 = Y2-Y1;
	...
	while (i < NoNodes-1)
	{
		...
		if (Shadows[i] == 1)
		{
	        ...
	        X2 = X[ShadowStart] + 100000.*np.sin((np.pi/180.)*ShadowAngle);
    	    Y2 = Y[ShadowStart] + 100000.*np.cos((np.pi/180.)*ShadowAngle);
        	dX12 = X2-X1;
	        dY12 = Y2-Y1;
        
        	CasterFlag = 0
        
        for j in range(i+1, NoNodes-2):
            ...
            if (XProd != 0):
                ...
                if ((S < 0) == XProdPos):
                    ...
                    #print ""
                    L = 1
                elif (((S > XProd) == XProdPos) or ((T > XProd) == XProdPos)):
                    #do nothing
                    #print ""
                    L = 1
                else:
                    print "Shadow in coastline detected!!"
                    print "i = ", i, "; j = ", j
                    Flag = 1
                    break
                    
        if (Flag == 1):
            Shadows[ShadowStart] = 2
        else:
            Shadows[ShadowEnd] = 3
	...
	for (i=1; i<NoNodes-1; ++i)
	{
		...
		//loop through vector to find interestion?
		for (j=i+2; j<NoNodes-1; ++j)
		{
		...
		if (Shadows[i] < 1)
		{
            ...
            //test if next point along coast is in shadow
            ShadowTest = ((X2 - X[i])*(Y[i+1] - Y[i]) - (Y2 - Y[i])*(X[i+1] - X[i]));
            if ((ShadowTest <= 0) && (abs(ShadowAngle-FluxOrientation[i]) < 90.)) Shadows[i] = 2;
