...
void Coastline::Initialise(int NodeSpacing, double CoastLength, double TheTrend, int StartBoundaryInput, int EndBoundaryInput)
{
	...
	while (Dist<CoastLength)
	{
		if (Dist==0)
		{
			...
			tempX = tempX + 10*MeanNodeSpacing*sin(Trend*M_PI/180) + 2*(-0.5+((double)rand()/RAND_MAX));
			tempY = tempY + 10*MeanNodeSpacing*cos(Trend*M_PI/180) + 2*(-0.5+((double)rand()/RAND_MAX));
			Dist += 1.0;
			X.push_back(tempX);
			Y.push_back(tempY);
			NoNodes += 1;
		...
		else
		{
			...
			tempX = tempX + MeanNodeSpacing*sin(Trend*M_PI/180) + 5.*(-0.5+((double)rand()/RAND_MAX));
			tempY = tempY + MeanNodeSpacing*cos(Trend*M_PI/180) + 5.*(-0.5+((double)rand()/RAND_MAX));
			X.push_back(tempX);
			Y.push_back(tempY);
			NoNodes += 1;
			Dist += MeanNodeSpacing;
...
void Coastline::IntersectionAnalysis()
{
	...
	*/
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		if (FluxOrientation[i]-FluxOrientation[i-1] < -135) 
		{
			printf("Wide CellWidth in %s at line %d\n",__func__,__LINE__);
			IntersectionFlag = 1;
			X.erase(X.begin()+i-1);
			Y.erase(Y.begin()+i-1);
			Fixed.erase(Fixed.begin()+i-1);
            ...
            if ((EndBoundary == 2) && (b == NoNodes-2))  MeshOrientation[b] = MeshOrientation[b-1];
		      ...
		      //Dsf can't be negative
				...
				if (e2[b] > 0) 
		     	...
		     	{
		     		if (Dsfb > ClosureDepth || Dsfb < 0) Dsfb = ClosureDepth-0.001;
		     	...
		     	{
		     		if (Dsfa > ClosureDepth || Dsfa < 0) Dsfa = ClosureDepth-0.001;
		 	...
		 	else if (CellType[i] == 2)
		 	{
		      ...
		      if (Dsft == 10)
		      {
				...
				{
					if ((Dsf[i] == ClosureDepth) && (Dsf[i+1] == ClosureDepth)) {}
					else if ((e1[i]+e2[i]) > (e1[i+1]+e2[i+1]) && (Dsf[i] >= Dsf[i+1]))
					{
...
////			     	//Vertices[b].push_back(XYNode);
			  	   ...
			  	   //find right boundary
		     		X0[i] = XL[i] - (ClosureDepth/ShorefaceSlope)*cos((M_PI/180.)*(Orientation[i]-e1[i]));
		 			Y0[i] = YL[i] + (ClosureDepth/ShorefaceSlope)*sin((M_PI/180.)*(Orientation[i]-e1[i]));
...
//		      }
		      ...
		      //find intersection
				...
				{	
					...
					if (((a > 0) && (Dsf[a] < ClosureDepth-0.001)) || ((b < NoNodes-1) && (Dsf[b] < ClosureDepth-0.001)))
					{
						if (Dsf[a] < Dsf[b])
						{
							if (CellType[a] != 4)
							{
								CellType[i] = 4;
					   		CurrentNode.ShorefaceDepth = Dsf[a]+0.001;
								CurrentNode.i = i;
								CoastlineQueue.push(CurrentNode);
								Dsf[i] = Dsf[a];
								CurrentNode.ShorefaceDepth = Dsf[a]-0.001;
						...
						else
						{
							...
							{
								CellType[i] = 4;
					   		CurrentNode.ShorefaceDepth = Dsf[b]+0.001;
								CurrentNode.i = i;
								CoastlineQueue.push(CurrentNode);
								Dsf[i] = Dsf[b];
								CurrentNode.ShorefaceDepth = Dsf[b]-0.001;
