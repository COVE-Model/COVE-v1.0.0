...
void Coastline::TransportSediment(double &TimeDelta, Wave TheWave, int FluxType, int RefDiffFlag, double FluxFraction)
{
	...
	*/
	...
	//Declare temporary variables
	double a, b, c, D_sf, D_sf_new, G, dt, dX, dY;
	//double TempLongshoreFluxLeft, TempLongshoreFluxRight; //, ExcessFluxLeft, ExcessFluxRight;
	double TempOrientation, TempCellWidth, Temp_e1, Temp_e2, Wb, TempVolumeChange, TempPositionChange;
	double PositionChange;
	double TotalVolumeOld, VolumeOut;
	int i, m, n; //iterators
	...
	while (!CoastlineQueue.empty())
	{
		...
		if (EvolveFlag[i] != 1)
		{
			...
			if (D_sf < ClosureDepth)
			{
				...
				//Call cubic solver
				if (VolumeChange[i] != 0) PositionChange[i] = SolveCubic(i);
		...
		else
		{
			...
			if (VolumeChange[i] != 0) PositionChange[i] = SolveQuadratic(i);
...
double Coastline::SolveQuadratic(i)
{
	//quadratic coefficients
	//note use of effective shoreface depth D_sf rather than "closure depth"
...
double Coastline::SolveCubic()
{
	...
	double a1, b1, c1, Q, Q3, R, R2, Theta, 2rootQ, aover3, x1, x2, x3;
	...
	if (R2 < Q3)
	{
		...
		2rootQ = -2.*sqrt(Q);
		x1 = 2rootQ*cos(Theta/3.) - aover3;
		x2 = 2rootQ*cos((Theta+2.*M_PI)/3.) - aover3;
		x3 = 2rootQ*cos((Theta-2.*M_PI)/3.) - aover3;
