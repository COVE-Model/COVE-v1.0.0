	...
	*/
	...
	for (i=1; i<NoNodes-1; ++i)
	{
		...
		if (Shadows[i] == 2)
		{
			...
			//TransformWave(j);
			TempWaveAngle = FluxOrientation[j]-90.-OffshoreWaveDirection;
			TempWaveHeight = OffshoreWaveHeight;
		...
		else if (Shadows[i] == 3)
		{
			...
			while ((j < i-1) && (j > 1))
			{
				...
				if (dX == 0 && dY < 0) 
				{
					ShadowZoneWaveDirection[j] = (OffshoreWaveDirection + 1.*(180.-TempWaveAngle))+90.;
					ShadowZoneWaveHeight[j] = TempWaveHeight*cos((M_PI/180.)*(OffshoreWaveDirection-180.));
				}
				else if (dX == 0 && dY > 0) 
				{
					ShadowZoneWaveDirection[j] = (TempWaveAngle + 1.*(360.-TempWaveAngle))+90.;
					ShadowZoneWaveHeight[j] = TempWaveHeight*cos((M_PI/180.)*(OffshoreWaveDirection-360.));
				}
				else if (dX > 0)
				{
					AngleDiff = (M_PI*0.5-atan(SpatialGradient))-((M_PI/180.)*(ShadowAngle));
					ShadowZoneWaveDirection[j] = OffshoreWaveDirection + 1.*90.*(180./M_PI)*sin(AngleDiff)*sin(AngleDiff);
					ShadowZoneWaveHeight[j] = OffshoreWaveHeight*cos(AngleDiff)*cos(AngleDiff)*cos(AngleDiff);
				}
				else if (dX < 0) 
				{
					AngleDiff = (M_PI*1.5-atan(SpatialGradient))-((M_PI/180.)*(ShadowAngle));
					ShadowZoneWaveDirection[j] = OffshoreWaveDirection + 1.*(180./M_PI)*AngleDiff;
					ShadowZoneWaveHeight[j] = OffshoreWaveHeight*cos(AngleDiff)*cos(AngleDiff)*cos(AngleDiff);
				}
				else 
				{
		...
		for (int j=i+2; j<NoNodes-1; ++j)
		{
			...
			else
			{
				...
				else
				{
					...
					//delete node if intersecting headland
					if (i==0)
					{
						NodesToRemove.push_back(j);
					}
				}
			}
		}
...
void Coastline::TransportSediment(double &TimeDelta, double MaxTimeStep, Wave TheWave, int FluxType, int RefDiffFlag, double FluxFraction)
{
	...
	*/
	...
	if ((fabs(OffshoreWaveDirection - Trend) < 10.) || (fabs(Trend-180.-OffshoreWaveDirection) < 10.)) return;
...
//					if (LongshoreFlux[i] < 0) 
//					{
//						LongshoreFlux[i] = 0;	//LongshoreFlux[NoNodes-1]*FluxFraction;
//						VolumeChange[i] = (LongshoreFlux[i-1])*(1.-LostFluxFraction)*TimeDelta;
//					}
